...
def score_matthews(TP, TN, FP, FN, tvdbg_p, tvdbg_l):
    ...
    # print 'tvdbg matthews[tvdbg_l] = {} =  TP[tvdbg_l]={} TN[tvdbg_l]={} FP[tvdbg_l]={} FN[tvdbg_l]={} num={}=({}-{}) den={}'.format(matthews[tvdbg_l], TP[tvdbg_l], TN[tvdbg_l], FP[tvdbg_l], FN[tvdbg_l], ((TP[tvdbg_l] * TN[tvdbg_l]) - (FP[tvdbg_l] * FN[tvdbg_l])), (TP[tvdbg_l] * TN[tvdbg_l]), (FP[tvdbg_l] * FN[tvdbg_l]), den)
    return matthews

def score_positive(TP, TN, FP, FN, tvdbg_p, tvdbg_l):
    score = [0 for l in range(len(TP))]
    # compute the score: TP / (TP + FP)
    for l in range(len(TP)):
        if TP[l] == 0:
            score[l] = 0
    # print 'tvdbg score[tvdbg_l] = {} =  TP[tvdbg_l]={} TN[tvdbg_l]={} FP[tvdbg_l]={} FN[tvdbg_l]={} num={}=({}-{}) den={}'.format(score[tvdbg_l], TP[tvdbg_l], TN[tvdbg_l], FP[tvdbg_l], FN[tvdbg_l], ((TP[tvdbg_l] * TN[tvdbg_l]) - (FP[tvdbg_l] * FN[tvdbg_l])), (TP[tvdbg_l] * TN[tvdbg_l]), (FP[tvdbg_l] * FN[tvdbg_l]), den)
        else:
